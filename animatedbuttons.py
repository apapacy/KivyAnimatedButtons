...
class AnimatedButtons(App):
    ...
    def __init__(self, **kvargs):
        super(AnimatedButtons, self).__init__(**kvargs)
        ...
        self.translation = Translation(
            self.lang, 'animatedbuttons', os.path.join(self.directory, 'data', 'locales')
        )

    def get_application_config(self):
        return super(AnimatedButtons, self).get_application_config(
                        '{}/%(appname)s.ini'.format(self.directory))

    def build_config(self, config):
        '''Создаёт файл настроек приложения animatedbuttons.ini.'''

        config.adddefaultsection('General')
        config.setdefault('General', 'language', 'ru')

    def set_value_from_config(self):
        '''Устанавливает значения переменных из файла настроек animatedbuttons.ini.'''

        self.config.read(os.path.join(self.directory, 'animatedbuttons.ini'))
    ...
    def show_about(self, *args):
        ...
        self.screen.ids.about.ids.label.text = \
            self.translation._(
                u'[size=20][b]Animated Buttons[/b][/size]\n\n'
                u'[b]Version:[/b] {version}\n'
                u'[b]License:[/b] MIT\n\n'
                u'[size=20][b]Developer[/b][/size]\n\n'
                u'[ref=SITE_PROJECT]'
                u'[color={link_color}]Ivanov Yuri[/color][/ref]\n\n'
                u'[b]Source code:[/b] '
